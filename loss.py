...
class PixelLoss(nn.Module):
    ...
    def __init__(self, p=1):
        ...
        self.p = p # p is the norm order

    def forward(self, canvas, gt, ignore_color=False):
        # ignore color implies we are setting the canvas and the image to its mean color respectively
        if ignore_color:
...
class VGGPerceptualLoss(torch.nn.Module):
    ...
    def forward(self, input, target, ignore_color=False):
        ...
        #this is for monochromatic images
        if input.shape[1] != 3:
